...
- A signed legal filing: identity check, confidentiality none - anyone can read it
- An encrypted tunnel to an unknown server: confidentiality check, identity none - private, but no proof of who's on the other end
...
Two separate AES keys - one per direction - so client-to-server and server-to-client are encrypted independently. The `SHA384` in `TLS_AES_256_GCM_SHA384` is the hash HKDF uses for this.
